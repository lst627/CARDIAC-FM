...
# new augmentation
class SplitAndMaskAsViews(object):
    def __init__(self, mask_leads_prob=0.5, p=1.0):
        self.mask_leads_prob = mask_leads_prob
        self.p = p
    ...
    def random_mask(self, x):
        ...
        new_x = x.clone()
        if self.p >= np.random.uniform(0, 1):
            survivors = np.random.uniform(0, 1, size=C) >= self.mask_leads_prob
            new_x[~survivors] = 0
        return new_x.float()
    ...
    def __call__(self, x):
        ...
        masked_half1 = self.random_mask(half1)
        masked_half2 = self.random_mask(half2)

        return masked_half1, masked_half2