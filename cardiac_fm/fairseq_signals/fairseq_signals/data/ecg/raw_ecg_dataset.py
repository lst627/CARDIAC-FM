    ...
    def __init__(
        ...
        **kwargs,
        ...
        if label_file is not None:
            ...
            self.label_array = np.load(label_file, allow_pickle=True)
...
class FileECGDataset(RawECGDataset):
        ...
        with open(manifest_path, "r") as f:
            ...
            for i, line in tqdm(enumerate(f)):
                ...
                if self.min_sample_size is not None and sz < self.min_sample_size:
                    skipped += 1
                    self.skipped_indices.add(str(items[0])[:-4])
                    continue
                if self.label_array is not None:
                    path = os.path.join(self.root_dir, str(items[0]))
                    ecg = scipy.io.loadmat(path)
                    label = self.label_array[ecg["idx"].squeeze()]
                    float_label = np.array([to_float(x) for x in label], dtype=float)
                    if np.isnan(float_label).any():
                        skipped += 1
                        self.skipped_indices.add(str(items[0])[:-4])
                        continue
    ...
    def __getitem__(self, index):
        ...
        if self.label:
            if self.label_array is not None:
                res["label"] = torch.from_numpy(self.label_array[ecg["idx"].squeeze()].astype(float))
            else:
