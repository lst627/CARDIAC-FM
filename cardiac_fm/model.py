...
class MMCL(nn.Module):
    def __init__(self, densenet_ckpt, mri_ckpt=None, ecg_ckpt=None):
        super(MMCL, self).__init__()
        self.mri_encoder = Three_plus_three_cnn_lstm(densenet_ckpt)
...
class CARDIACFM_ECGMRI(nn.Module):
    ...
    def __init__(self, ecgfm_ckpt, densenet_ckpt, device, 
        ...
        self.mri_encoder_multi = Three_plus_three_cnn_lstm(densenet_ckpt)
...
def CHARGE_AF(
    ...
    cursmoke : int
        1 = current smoker, 0 = no
    htnmed : int
        1 = on hypertension medication, 0 = no
    prevdm : int
        1 = diabetes, 0 = no
    prevhf : int
        1 = previous heart failure, 0 = no
    prevmi : int
        1 = history of MI, 0 = no
...
def PREVENT_HF(df):
    """
    Compute PREVENT-HF score (probability) based on the R code you provided.
    ...
    logodds_w = (
        ...
        + 0.0557087 * egfr_high
        + 0.3534442 * htnmed_i
        - 0.0981511 * htnmed_i * sbp_high
        - 0.0946663 * age10 * sbp_high
        - 0.3581041 * age10 * prevdm_i
        - 0.1159453 * age10 * cursmoke_i
    ...
    logodds_m = (
        ...
        + 0.3634461 * sbp_high
        + 0.923776 * prevdm_i
        + 0.5023736 * cursmoke_i
        - 0.0485841 * bmi_low
        + 0.3726929 * bmi_high
        + 0.6926917 * egfr_low
        + 0.0251827 * egfr_high
        + 0.2980922 * htnmed_i
        - 0.0497731 * htnmed_i * sbp_high
        - 0.1289201 * age10 * sbp_high
        - 0.3040924 * age10 * prevdm_i
        - 0.1401688 * age10 * cursmoke_i
...
def CARDIACFM_ECG_Risk_AF(df, seed):
    ...
    coefs = {
        1: (-4.057,  8.468, 27.948),
        2: (-3.715, 12.537, 34.139),
        3: (-4.049,  5.555, 27.302),
        4: (-3.727,  6.621, 31.046),
    }
    ...
    score = (
        intercept
        + beta_m * df["model_score"].values
        + beta_r * df["risk_score"].values
    )

    return score

def CARDIACFM_ECGMRI_Risk_AF(df, seed):
    if seed not in (1, 2, 3, 4):
        raise ValueError("seed must be one of 1, 2, 3, 4")

    coefs = {
        1: (-3.962, 12.918, 28.000),
        2: (-5.026,  7.182, 14.200),
        3: (-3.906, 13.615, 30.688),
        4: (-3.675,  8.946, 36.113),
    }
...
def CARDIACFM_ECG_Risk_HF(df, seed):
    ...
    coefs = {
        1: (-5.477, 11.142, 12.457),
        2: (-5.621, 21.670, 16.661),
        3: (-5.632, 13.969, 17.031),
        4: (-5.820, 15.740, 16.020),
    }
...
def CARDIACFM_ECGMRI_Risk_HF(df, seed):
    if seed not in (1, 2, 3, 4):
        raise ValueError("seed must be one of 1, 2, 3, 4")

    coefs = {
        1: (-5.551, 14.222, 15.232),
        2: (-5.693, 13.388, 18.203),
        3: (-5.539,  9.050, 18.581),
        4: (-5.648,  7.891, 18.258),
    }
