...
class mydataset(torch.utils.data.Dataset):
    def __init__(self, datapath, disease, weighted_sampling=False):

        df = pd.read_csv(datapath)
        n = len(df) 
        print('Total training data:', n)
        ...
        for i in tqdm(range(n)):
            ...
            mri_label = 1 if df.iloc[i]['af_1'] == 1.0 or df.iloc[i]['af_2'] == 1.0 or df.iloc[i]['af_3'] == 1.0 or df.iloc[i]['af_4'] == 1.0 or df.iloc[i]['af_5'] == 1.0 else 0
            frame_dir = osp.join(root_dir, eid_visit)
            if not osp.exists(os.path.join(frame_dir, 'la_4ch_crop.npy')):
                warnings.warn(f"{os.path.join(frame_dir, 'la_4ch_crop.npy')} NOT FOUND! This sample will be excluded.")
                continue
            
            data_ori = np.load(os.path.join(frame_dir, 'la_4ch_crop.npy'))
            data_ori = np.transpose(data_ori, (2, 0, 1))  # Shape: (50, 96, 96)
            ...
            try:
                ...
                self.class_counts[mri_label] += 1

            except:
                pass

        self.total_samples = sum(self.class_counts)
        self.class_weights = [self.total_samples / self.class_counts[i] for i in range(len(self.class_counts))]
...
def run(disease, train_sax, test_sax):
    ...
    # dataloader
    train_loader = DataLoader(dataset=train_dataset, batch_size=16, num_workers=0, sampler=train_sampler)
    test_loader = DataLoader(dataset=test_dataset, batch_size=16, num_workers=0, sampler=test_sampler)
    ...
    output_path = './output/cnn_lstm_test/'
    ...
    test_acc = 0.0
    best_acc = 0.0
    if (not os.path.exists(output_path + out_csv_name)) and epoch_resume == 0:
...
# Training: CUDA_VISIBLE_DEVICES=0,1,2,3 python -m torch.distributed.launch --nproc_per_node=4 train.py