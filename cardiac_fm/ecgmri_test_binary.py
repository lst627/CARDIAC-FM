...
def test(batch_size, args):
    ...
    mris_test = f"{args.mris_csv_dir}/valid.csv"
    ecgs_tsv_dir = args.ecg_tsv_dir
    labels_dir = f"{args.label_dir}/y.npy"

    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print(f"Using device: {device}")
    testset = ECGMRIDataset(mris_test, ecgs_tsv_dir, mris_dir, labels_dir, split = "valid")
...
if __name__=="__main__":
    ...
    #wandb.init(
        #entity="lifm6-fred-hutch",
        #project="downstream_test",
        #name=f"{args.model_name}_{args.label}_{args.length}_{args.type}"
    #)
